...
export class UserService {
  ...
  login(options: UserLoginOptions): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      this.http
        .post(environment.url + 'jwt-auth/v1/token', {
  ...
  public get onLoggedIn(): Observable<UserData> {
    return this.data.pipe(first(data => !!data));
  }

  public get onLoggedOut(): Observable<UserData> {
    return this.data.pipe(skip(1), first(data => !data));
  }
  ...
  validateToken() {
    ...
    this.http
      .post(
        environment.url + 'jwt-auth/v1/token/validate',
        {},
        { headers: headers }
      )
      .subscribe(
        (res: HttpResponse<any>) => {
          if (res && res['code'] == 'jwt_auth_valid_token')
            console.debug('UserService => Auth token valid');
          this.setUserData(data);
        },
        (error: HttpErrorResponse) => {
          ...
          if (
            error &&
            error.error &&
            (error.error.code == 'jwt_auth_failed' ||
              error.error.code == 'jwt_auth_invalid_token')
          )
            this.logout();
        }
      );
